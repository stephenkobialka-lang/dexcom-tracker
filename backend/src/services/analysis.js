...
Please provide:
1. INSIGHTS: A clear, parent-friendly summary of the key patterns you see in this data (3-4 sentences).
2. SUGGESTIONS: 3-4 specific, actionable suggestions for the parents to discuss with their endocrinologist.
...
Do not include markdown code fences. Return only valid JSON.`;
  ...
  try {
    ...
    console.log('[Analysis] API Key prefix:', process.env.ANTHROPIC_API_KEY?.substring(0, 10));
    ...
    console.log('[Analysis] Raw response:', text.substring(0, 200));
    ...
    if (err.status) console.error('[Analysis] Status:', err.status);
    if (err.error) console.error('[Analysis] Error detail:', JSON.stringify(err.error));
